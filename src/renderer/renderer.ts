...
function asHex(num: number): string {
...
function updateCurrentBytes(addressesContainer: HTMLElement | null, valuesContainer: HTMLElement | null, rows: number, columns: number, baseAddress: number | null, offset: number, currentBytes: string[], windowIsScaling: boolean) : void {
...
window.memoryAPI.start().then(async () => {
    ...
    if (valuesContainer && addressesContainer) observer.observe(valuesContainer);
